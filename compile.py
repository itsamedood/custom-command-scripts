...
def compile_scripts(_files: list[str], _exclude: list[str]) -> None:
  for f in _files:
    if f == "self":
      ...
      system(f"pyinstaller --onefile --distpath ./out/ccsc --name ccsc {THIS}/compile.py")
      print(f"\033[1;32mCompiled compile.py!\033[0;0m")


    else:
      if f not in _exclude and not f[:1] == "_":
        print(f"\033[1;33mCompiling {f}.py...\033[0;0m")
        system(f"pyinstaller --onefile --distpath ./out/{f} --name {f} {PATH}/{f}.py")
...
def cleanup() -> None: system("rm *.spec")  # Only side affect of using PyInstaller.


if __name__ == "__main__": run(analyze_flags()); cleanup()
